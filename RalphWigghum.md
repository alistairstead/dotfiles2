...
Supporting research from anthropic: https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agentsaud

Anthropic example prompts https://github.com/anthropics/claude-quickstarts/tree/main/autonomous-coding/prompts
...
Skills:
- The agent should use jq to interact with the prd.json file.
- The agent should select the next appropriate task not just the next in the list
- Add a command to help build the project specification and then prd.json

I like the simplicity of the prompts in the bash example. Only adopt features from the Anthropic example prompts if they will materially improve the functionality of the agent. Guiding the loop and parent claude to use sub agents is seen as a good practice.
